...
export async function GET(req: NextRequest, context: any) {
  ...
  const rows: any[] = await prisma.walletTransaction.findMany({ where: { loadId: load.id }, orderBy: { createdAt: 'asc' } });
  ...
  const feeTxs: any[] = await prisma.walletTransaction.findMany({ where: { loadId: load.id, type: { contains: 'FEE' } } });
  const totalFees = feeTxs.reduce((s, t) => s + Math.abs(t.amountCents || 0), 0);

    // approval timestamp: first LOAD_GROSS txn
  const approvalTx = rows.find((r: any) => String(r.type ?? '').toUpperCase().includes('LOAD_GROSS'));
